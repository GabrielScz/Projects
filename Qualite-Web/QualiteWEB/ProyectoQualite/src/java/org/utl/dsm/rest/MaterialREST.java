...
import org.utl.dsm.controller.ControllerMaterial;
...
public class MaterialREST {
    ...
    public Response insert(@FormParam("datos") @DefaultValue("") String datos,
        ...
        try {
            if (objCL.validarToken(token)) {
                m = gson.fromJson(datos, Material.class);
                objCM.insert(m);
                out = gson.toJson(m);
            } else {
                out = """
    ...
    public Response getAll(@FormParam("token") @DefaultValue("") String token) {
        ...
        try {
            if (objCL.validarToken(token)) {
                try {
                    ControllerMaterial objCM = new ControllerMaterial();
                    List<Material> materiales = objCM.getAll();
                    Gson gs = new Gson();
    ...
    public Response search(@QueryParam("busqueda") @DefaultValue("Car") String busqueda) {
        ...
        try {
            ControllerMaterial objCM = new ControllerMaterial();
            List<Material> materiales = objCM.search(busqueda);
            Gson gs = new Gson();
            out = gs.toJson(materiales);
        } catch (Exception ex) {
            System.out.println(ex.toString());
