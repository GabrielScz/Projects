...
import org.utl.dsm.controller.ControllerMica;
...
public class MicaREST {
    ...
    public Response insert(@FormParam("datos") @DefaultValue("") String datos,
        ...
        TipoMica m = null;
        String out = null;
        ControllerMica objCM = new ControllerMica();
        ControllerLogin objCL = new ControllerLogin();

        try {
            if (objCL.validarToken(token)) {
                m = gson.fromJson(datos, TipoMica.class);
                objCM.insert(m);
                out = gson.toJson(m);
            } else {
                out = """
    ...
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAll(@FormParam("token") @DefaultValue("") String token) {
        ...
        try {
            if (objCL.validarToken(token)) {
                try {
                    ControllerMica objCM = new ControllerMica();
                    List<TipoMica> micas = objCM.getAll();
                    Gson gs = new Gson();
                    out = gs.toJson(micas);
                } catch (Exception ex) {
    ...
    public Response search(@QueryParam("busqueda") @DefaultValue("Car") String busqueda) {
        ...
        try {
            ControllerMica objCM = new ControllerMica();
            List<TipoMica> micas = objCM.search(busqueda);
            Gson gs = new Gson();
            out = gs.toJson(micas);
        } catch (Exception ex) {
            System.out.println(ex.toString());
