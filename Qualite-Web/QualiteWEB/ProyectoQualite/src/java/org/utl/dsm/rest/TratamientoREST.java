...
import org.utl.dsm.controller.ControllerTratamiento;
...
public class TratamientoREST {
    ...
    public Response insert(@FormParam("datos") @DefaultValue("") String datos,
        ...
        try {
            if (objCL.validarToken(token)) {
                t = gson.fromJson(datos, Tratamiento.class);
                objCT.insert(t);
                out = gson.toJson(t);
            } else {
                out = """
    ...
    public Response getAll(@FormParam("estatus") @DefaultValue("1") String estatus,
        ...
        try {
            if (objCL.validarToken(token)) {
                try {
                    ControllerTratamiento objCT = new ControllerTratamiento();
                    List<Tratamiento> tratamientos = objCT.getAll(estatus);
                    Gson gs = new Gson();
    ...
    public Response search(@QueryParam("busqueda") @DefaultValue("Car") String busqueda) {
        ...
        try {
            ControllerTratamiento objCT = new ControllerTratamiento();
            List<Tratamiento> tratamientos = objCT.search(busqueda);
            Gson gs = new Gson();
            out = gs.toJson(tratamientos);
        } catch (Exception ex) {
            System.out.println(ex.toString());
